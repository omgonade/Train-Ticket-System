...
#########################################################Function Declaraion#################################################
        ...
        def Travel_Cost():
            if (var9.get() == "Delhi->Kanpur" and var1.get() == 1 and var4.get() == 1):
                ...
                Return = float(2*var11.get())
                Adult_Tax = "Rs.", str('%.2f'% ((Tcost * (Single+Return)) * 0.05))
                Adult_Fees = "Rs.", str('%.2f'% (Tcost * (Single+Return)))
                TotalCost = "Rs.", str('%.2f' % ((Tcost * (Single+Return)) + ((Tcost * (Single+Return)) * 0.05)))
...
########################################################################################################################################################
        ...
        entSubtotal = Entry(bottomLeft1, font=('arial', 16, 'bold'), textvariable=SubTotal, bd=8, width=28).grid(row=1,column=1,sticky=W)
        lblTotal=Label(bottomLeft1,font=('arial',16,'bold'),text="Total",bd=8).grid(row=2,column=0,sticky=W)
        entTotal = Entry(bottomLeft1, font=('arial', 16, 'bold'), textvariable=Total, bd=8, width=28).grid(row=2,column=1,sticky=W)
########################################################################################################################################################
